...
```{r setup, include=FALSE}
  ...
  MARSDB_con <- dbConnect(odbc(), dsn = params$database, uid = Sys.getenv("shiny_uid"), pwd = Sys.getenv("shiny_pwd") )
  ...
  fieldwork.test_wic_smps <- data.frame(matrix(NA, 0, 2))
...
# gather current wic orders and comments from MARS_Data
   ...
   wic_workorders <- dbGetQuery(MARSDB_con, "SELECT * FROM fieldwork.test_wic_workorders ")
   wic_comments <- dbGetQuery(MARSDB_con, "SELECT * FROM fieldwork.test_wic_comments ")
...
# Append the new WIC to wic_workorders
  ...
  dbWriteTable (MARSDB_con, SQL("fieldwork.test_wic_workorders"),new_wic,append= TRUE, row.names = FALSE)
...
# Update the workorders in wic_workorders that have been changed in cityworks DB 
   ...
   if(nrow(changed_wic_cityworks) > 0 & params$write == TRUE){
     ...
     sql_string <- "UPDATE fieldwork.test_wic_workorders SET wo_initiatedate = '%s', location = '%s',  wo_xcoordinate = %s,wo_ycoordinate= %s,facility_id = '%s' WHERE workorder_id = %s;"
...
# update comments and add new comments to the new workorders
  ...
  dbWriteTable (MARSDB_con, SQL("fieldwork.test_wic_comments"),new_wo_comments,append= TRUE, row.names = FALSE)
   ...
   if(nrow(changed_comments_cityworks) > 0 & params$write == TRUE){
      ...
      sql_string <- "delete from fieldwork.test_wic_comments WHERE workorder_id = %s;"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, changed_comments_cityworks$workorder_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_comments"),changed_comments_cityworks,append= TRUE, row.names = FALSE)
...
```{r Section 7- displaying tables of parcels attached to the new and changed work orders and write to DB ,echo=FALSE, results='asis'}
     ...
     if ( !is.null(wic_parcels)) {
      ...
      if (!is.null(wic_parcels) & params$write == TRUE){
        ...
        sql_string <- "delete from fieldwork.test_wic_parcels WHERE workorder_id = %s;"
        dbSendStatement(MARSDB_con, paste(sprintf(sql_string, wic_parcels$workorder_id), collapse=""))
    
        dbWriteTable (MARSDB_con, SQL("fieldwork.test_wic_parcels"),wic_parcels,append= TRUE, row.names = FALSE)
...
###Taylor says: Filter to complete cases (ie, no NAs or gaps in data) and write to DB
  ...
  ## section 5: categorizing the SMPs to pre, during, or post construction
    ...
    #setting the lookup_id's default in smpmilestone to 4
        ...
        fieldwork.test_wic_smps <- smp_milestones %>% select(workorder_id,smp_id, wic_facility_id, buffer_ft, phase_lookup_uid)
        fieldwork.test_wic_smps['system_id'] <- gsub('-\\d+$','',fieldwork.test_wic_smps$smp_id ) 
      
        if (nrow(fieldwork.test_wic_smps) > 0 & params$write == TRUE){
          
          sql_string <- "delete from fieldwork.test_wic_smps WHERE workorder_id = %s;"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, fieldwork.test_wic_smps$workorder_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_smps"),fieldwork.test_wic_smps,append= TRUE, row.names = FALSE)
        }
...
```{r section 9- SMP map data update, include = FALSE, eval= nrow(fieldwork.test_wic_smps)>0}

  new_smp_list <- fieldwork.test_wic_smps %>% 
  select(smp_id) %>%
  distinct()
  new_smp_spatial <- SMP %>%
  ...
  names(smp_wkt) <- c("smp_id","wkt")
   ...
   if (nrow(smp_wkt) > 0 & params$write == TRUE){
          ...
          sql_string <- "delete from fieldwork.test_wic_smp_wkt WHERE smp_id = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, smp_wkt$smp_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_smp_wkt"),smp_wkt,append= TRUE, row.names = FALSE)
    }
...
```{r section 10- WIC parcels map data update, include = FALSE, eval= nrow(fieldwork.test_wic_smps)>0}

 wic_parcels_spatial <- Parcels_WIC_Filterd %>%
   inner_join(fieldwork.test_wic_smps, by=c("FACILITYID"="wic_facility_id")) %>%
 ...
 wic_parcels_wkt['wkt'] <- wic_parcels_wkt_df
  ...
  if (nrow(wic_parcels_wkt) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.test_wic_parcels_wkt WHERE smp_id = '%s' and address = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, wic_parcels_wkt$smp_id, wic_parcels_wkt$address), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_parcels_wkt"),wic_parcels_wkt,append= TRUE, row.names = FALSE)
  }
...
# Create a new wkt table for the bulding footprint by updating the WKT column
  parcel_building_footprint <- wic_parcels_wkt %>%
    ...
    for (i in 1:nrow(building_footprint)){
      ...
      dist[i] <- st_distance(building_footprint[i,], new_smp_spatial %>%
                                 filter(SMP_ID == building_footprint_address %>%
                                          filter(address == building_footprint_address[i,"address"]) %>%
                                          select(smp_id) %>%
                                          distinct %>%
                                          pull))/0.3048
 ...
 #write to DB
   if (nrow(parcel_building_footprint) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.test_wic_buildingfootprint_wkt WHERE smp_id = '%s' and address = '%s';"
          ...
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_buildingfootprint_wkt"),parcel_building_footprint,append= TRUE, row.names = FALSE)
  }
...
```{r section 11- all parcels (wic and non-wic) map data update, include = FALSE, eval= nrow(fieldwork.test_wic_smps)>0 }
 ...
 wic_all_parcels_wkt['wkt'] <- wic_all_parcels_wkt_df
  ...
  if (nrow(wic_all_parcels_wkt) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.test_wic_all_parcels_wkt WHERE system_id = '%s' and address = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, wic_all_parcels_wkt$system_id, wic_all_parcels_wkt$address), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.test_wic_all_parcels_wkt"),wic_all_parcels_wkt,append= TRUE, row.names = FALSE)
    }
