...
### Section 3: shiny work-A navbarpage with two tabs, within tab 1 there will be 3 tabsets with 3 tables of data and corresponding download button on the sidebar
    ...
    server <- function(input, output) {
      ...
      output$map <- renderLeaflet({
        ...
        leaflet()%>%addTiles(options = providerTileOptions(minZoom = 14, maxZoom = 18)) %>%  addPolygons(data = filter(parcel_spatial, smp_id == input$smp_id & buffer_ft == input$buffer ), group = "Parcels") %>% addPolygons(data=filter(smp_spatial, SMP_ID == input$smp_id  ), color = "red", group = "SMP") 
