...
```{r setup, include=FALSE}
  ...
  fieldwork.wic_smps <- data.frame(matrix(NA, 0, 2))
...
# update comments and add new comments to the new workorders
  ...
  dbWriteTable (MARSDB_con, SQL("fieldwork.wic_comments"),new_wo_comments,append= TRUE, row.names = FALSE)
   ...
   if(nrow(changed_comments_cityworks) > 0 & params$write == TRUE){
      ...
      sql_string <- "delete from fieldwork.wic_comments WHERE workorder_id = %s;"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, changed_comments_cityworks$workorder_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_comments"),changed_comments_cityworks,append= TRUE, row.names = FALSE)
...
```{r Section 7- displaying tables of parcels attached to the new and changed work orders and write to DB ,echo=FALSE, results='asis'}
     ...
     if ( !is.null(wic_parcels)) {
      ...
      if (!is.null(wic_parcels) & params$write == TRUE){
        ...
        sql_string <- "delete from fieldwork.wic_parcels WHERE workorder_id = %s;"
  ...
  ## section 5: categorizing the SMPs to pre, during, or post construction
    ...
    #setting the lookup_id's default in smpmilestone to 4
        ...
        if (nrow(fieldwork.wic_smps) > 0 & params$write == TRUE){
          ...
          sql_string <- "delete from fieldwork.wic_smps WHERE workorder_id = %s;"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, fieldwork.wic_smps$workorder_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_smps"),fieldwork.wic_smps,append= TRUE, row.names = FALSE)
        }
...
```{r section 9- SMP map data update, include = FALSE, eval= nrow(fieldwork.wic_smps)>0}

  new_smp_list <- fieldwork.wic_smps %>% 
  select(smp_id) %>%
  distinct()
  new_smp_spatial <- SMP %>%
  ...
  names(smp_wkt) <- c("smp_id","wkt")
   ...
   if (nrow(smp_wkt) > 0 & params$write == TRUE){
          ...
          sql_string <- "delete from fieldwork.wic_smp_wkt WHERE smp_id = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, smp_wkt$smp_id), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_smp_wkt"),smp_wkt,append= TRUE, row.names = FALSE)
    }
...
```{r section 10- WIC parcels map data update, include = FALSE, eval= nrow(fieldwork.wic_smps)>0}

 wic_parcels_spatial <- Parcels_WIC_Filterd %>%
   inner_join(fieldwork.wic_smps, by=c("FACILITYID"="wic_facility_id")) %>%
 ...
 wic_parcels_wkt['wkt'] <- wic_parcels_wkt_df
  ...
  if (nrow(wic_parcels_wkt) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.wic_parcels_wkt WHERE smp_id = '%s' and address = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, wic_parcels_wkt$smp_id, wic_parcels_wkt$address), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_parcels_wkt"),wic_parcels_wkt,append= TRUE, row.names = FALSE)
  }
 ...
 #update the footprint polygons
  ...
  names(building_footprint_address) <- c("address","wkt")
  building_footprint <- building_footprint_address
  
# Create a new wkt table for the bulding footprint by updating the WKT column
  parcel_building_footprint <- wic_parcels_wkt %>%
    select(-wkt) %>%
    inner_join(building_footprint, by="address")
 #write to DB
   if (nrow(parcel_building_footprint) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.wic_buildingfootprint_wkt WHERE smp_id = '%s' and address = '%s';"
          ...
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_buildingfootprint_wkt"),parcel_building_footprint,append= TRUE, row.names = FALSE)
  }
...
```{r section 11- all parcels (wic and non-wic) map data update, include = FALSE, eval= nrow(fieldwork.wic_smps)>0 }
 ...
 wic_all_parcels_wkt['wkt'] <- wic_all_parcels_wkt_df
  ...
  if (nrow(wic_all_parcels_wkt) > 0 & params$write == TRUE){
          sql_string <- "delete from fieldwork.wic_all_parcels_wkt WHERE system_id = '%s' and address = '%s';"
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, wic_all_parcels_wkt$system_id, wic_all_parcels_wkt$address), collapse=""))
          dbWriteTable (MARSDB_con , SQL("fieldwork.wic_all_parcels_wkt"),wic_all_parcels_wkt,append= TRUE, row.names = FALSE)
    }
