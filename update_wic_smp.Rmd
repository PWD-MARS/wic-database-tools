...
```
Public SMPs were intersected with parcels associated with new/changed WIC work orders within 25, 50, and 100 ft of distance from them. This resulted in `r nrow( smp_milestones %>% filter(phase_lookup_uid==1))` SMP attachment in Pre-construction phase, `r nrow( smp_milestones %>% filter(phase_lookup_uid==2))` SMP attachment during-construction phase, `r nrow( smp_milestones %>% filter(phase_lookup_uid==3))` SMP attachment in Post-construction phase, and `r nrow( smp_milestones %>% filter(phase_lookup_uid==4))` SMP attachment with unknown construction phase.

```{r section 8- displaying summary table of SMP association, include=FALSE, eval= nrow(smp_milestones)>0}

      smp_milestones$construction_phase <- replace(smp_milestones$construction_phase, smp_milestones$construction_phase == 1, "Pre-construction" )
      smp_milestones$construction_phase <- replace(smp_milestones$construction_phase, smp_milestones$construction_phase == 2, "During-construction" )
      smp_milestones$construction_phase <- replace(smp_milestones$construction_phase, smp_milestones$construction_phase == 3, "Post-construction" )
      smp_milestones$construction_phase <- replace(smp_milestones$construction_phase, smp_milestones$construction_phase == 4, "Unknown" )

      smp_milestones <- smp_milestones %>% select(workorder_id, smp_id, construction_phase, buffer_ft)
...
```{r section 9- DB disconnect, include=FALSE}
