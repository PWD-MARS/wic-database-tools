...
```
Public SMPs were intersected with parcels associated with new/changed WIC work orders within 25, 50, and 100 ft of distance from them. This resulted in `r nrow( smp_miliestones %>% filter(phase_lookup_uid==1))` SMP attachment in Pre-construction phase, `r nrow( smp_miliestones %>% filter(phase_lookup_uid==2))` SMP attachment during-construction phase, `r nrow( smp_miliestones %>% filter(phase_lookup_uid==3))` SMP attachment in Post-construction phase, and `r nrow( smp_miliestones %>% filter(phase_lookup_uid==4))` SMP attachment with known construction phase.

```{r section 8- displaying summary table of SMP association at buffer 25ft, include=FALSE, eval= nrow(output_25)>0}

      smp_milestones['construction_phase'] <- replace(smp_milestones$phase_lookup_uid, smp_milestones$phase_lookup_uid == 1, "Pre-construction" )
      smp_milestones['construction_phase'] <- replace(smp_milestones$phase_lookup_uid, smp_milestones$phase_lookup_uid == 2, "During-construction" )
      smp_milestones['construction_phase'] <- replace(smp_milestones$phase_lookup_uid, smp_milestones$phase_lookup_uid == 3, "Post-construction" )
      smp_milestones['construction_phase'] <- replace(smp_milestones$phase_lookup_uid, smp_milestones$phase_lookup_uid == 4, "Unknown" )
