...
```{r Section 3- attach parcels to new and changed workorder ,include=FALSE, eval= nrow(wo_noparcels) > 0}
...
The new and modified work orders (total = `r nrow(wo_noparcels)` ) were searched in GIS database by their facility id, location, and XY coordinate. This has resulted in `r nrow(wic_parcels)` parcel attachment.  


```{r Section 5- displaying tables of parcels attached to the new and changed work orders and write to DB ,echo=FALSE, results='asis'}

     if ( nrow(wic_parcels) > 0 ) {
    
        kable(wic_parcels, caption = "New and Modified WIC Parcels")
...
```{r Section 6- SMP intersect, include=FALSE, eval= nrow(wic_parcels) > 0 }
...
```
Public SMPs were intersected with parcels associated with new/changed WIC work orders within 25, 50, and 100 ft of distance from them. This resulted in `r nrow(output_25)`, `r nrow(output_50)`, and `r nrow(output_100)` SMP_WIC attachment, respectively. 


```{r section 7- displaying summary table of SMP association at buffer 25ft, include=FALSE, eval= nrow(output_25)>0}

  output_25['construction_phase'] <- replace(output_25$phase_lookup_uid, output_25$phase_lookup_uid == 1, "Pre-construction" )
  output_25['construction_phase'] <- replace(output_25$phase_lookup_uid, output_25$phase_lookup_uid == 1, "During-construction" )
  output_25['construction_phase'] <- replace(output_25$phase_lookup_uid, output_25$phase_lookup_uid == 1, "Post-construction" )
  output_25['construction_phase'] <- replace(output_25$phase_lookup_uid, output_25$phase_lookup_uid == 1, "Unknown" )
  output_25 <- output_25 %>% select(smp_id, construction_phase)
  
  kable(nrow(output_25), caption = "SMPs within 25 ft of new/changed WIC parcels ")

```

```{r  section 8- displaying summary table of SMP association at buffer 50ft, include=FALSE, eval= nrow(output_50)>0}

  output_50['construction_phase'] <- replace(output_50$phase_lookup_uid, output_50$phase_lookup_uid == 1, "Pre-construction" )
  output_50['construction_phase'] <- replace(output_50$phase_lookup_uid, output_50$phase_lookup_uid == 1, "During-construction" )
  output_50['construction_phase'] <- replace(output_50$phase_lookup_uid, output_50$phase_lookup_uid == 1, "Post-construction" )
  output_50['construction_phase'] <- replace(output_50$phase_lookup_uid, output_50$phase_lookup_uid == 1, "Unknown" )
  output_50 <- output_50 %>% select(smp_id, construction_phase)
  
  kable(nrow(output_50), caption = "SMPs within 50 ft of new/changed WIC parcels ")

```

```{r section 9- displaying summary table of SMP association at buffer 100ft, include=FALSE, eval= nrow(output_100)>0}

  output_100['construction_phase'] <- replace(output_100$phase_lookup_uid, output_100$phase_lookup_uid == 1, "Pre-construction" )
  output_100['construction_phase'] <- replace(output_100$phase_lookup_uid, output_100$phase_lookup_uid == 1, "During-construction" )
  output_100['construction_phase'] <- replace(output_100$phase_lookup_uid, output_100$phase_lookup_uid == 1, "Post-construction" )
  output_100['construction_phase'] <- replace(output_100$phase_lookup_uid, output_100$phase_lookup_uid == 1, "Unknown" )
  output_100 <- output_100 %>% select(smp_id, construction_phase)
  
  kable(nrow(output_100), caption = "SMPs within 100 ft of new/changed WIC parcels ")
