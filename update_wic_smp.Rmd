  ...
  ## section 5: categorizing the SMPs to pre, during, or post construction
    ...
    #setting the lookup_id's default in smpmilestone to 4
        ...
        if (nrow(fieldwork.wic_smps) > 0 & params$write == TRUE){
          ...
          dbSendStatement(MARSDB_con, paste(sprintf(sql_string, fieldwork.wic_smp$workorder_id), collapse=""))
...
```{r section 9- trimming for the showing the table in kable, include = FALSE, eval= nrow(smp_milestones)>0}
...
```{r Section 10- showing the table in kable ,echo=FALSE, results='asis'}
