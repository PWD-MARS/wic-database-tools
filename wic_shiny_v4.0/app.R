...
ui <- tagList(useShinyjs(), navbarPage("WIC App",
  ...
  tabPanel("System WIC Status",
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        conditionalPanel(
          condition = "input.date_range == 'Fiscal Quarter'",
          fluidRow(column(
            12,
      ...
      mainPanel(
        ...
        reactableOutput("wic_table") %>% withSpinner(color = "#0dc5c1"),
        width = 9
      )
    )
  ),
  ...
  tabPanel("WIC Investigation",
    value = "wic_insight",
    fluidRow(
      column(
        7,
        box(
          width = NULL, solidHeader = TRUE, status = "primary", class = "custom-box",
          fluidRow(
...
server <- function(input, output, session) {
  ...
  observeEvent(rv$row_wic_table(), {
    if (!is.null(rv$row_wic_table())) {
      ...
      # delay so that the basemap develops first. This ensures the map doesn't crash
  ...
  output$map <- renderLeaflet({
    ...
    map <- leaflet() %>%
      ...
      addLegend(
        colors = c("#000000", "#00ffbf", "red", "purple"),
        labels = c("System", "Buffer Area (100ft)", "WIC Property Line", "WIC Building Footprint")
      ) %>%
      addDrawToolbar(
        polylineOptions = drawPolylineOptions(metric = FALSE, feet = TRUE),
